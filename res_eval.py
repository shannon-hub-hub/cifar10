...
plt.savefig("results/res_training_and_validation_loss.png", dpi=150, bbox_inches="tight")
plt.show()

plt.figure(figsize=(8,5))
plt.plot(res_train_accuracies, label="Training Accuracy")
plt.plot(res_val_accuracies, label="Validation Accuracy")
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("Training and Validation Accuracies")
plt.legend()
os.makedirs("results", exist_ok=True)
plt.savefig("results/res_training_and_validation_accuracies.png", dpi=150, bbox_inches="tight")
plt.show()

from sklearn.metrics import confusion_matrix
import numpy as np

res_labels, res_preds = get_test_predictions(net_res)

# 1. compute confusion matrix and find top 5 confused pairs automatically
res_cm = confusion_matrix(res_labels, res_preds)
cm_off_diag = res_cm.copy()
...
disp = ConfusionMatrixDisplay(confusion_matrix=res_cm, display_labels=classes)
disp.plot(ax=ax, cmap="Blues", xticks_rotation=45, values_format="d")
plt.title("ResNet Confusion Matrix")
plt.tight_layout()
plt.savefig("results/res_confusion_matrix.png", dpi=150, bbox_inches="tight")
plt.show()
...
with torch.no_grad():
    for inputs, labels in testloader:
        ...
        for img, pred, truth in zip(inputs[wrong], predicted[wrong], labels[wrong]):
            res_misclassified.append((img.cpu(), pred.cpu().item(), truth.cpu().item()))
...
for row, (true_idx, pred_idx) in enumerate(top_pairs):
    examples = show_examples_for_pair(res_misclassified, true_idx, pred_idx, n=4)
    for col in range(4):
        ax = axes[row, col]
        if col < len(examples):
...
plt.tight_layout()
plt.savefig("results/res_misclassified.png", dpi=150, bbox_inches="tight")
plt.show()
